...
void genOffset()
{
  ...
  const Int_t mNTrays = 120;
   ...
   double tmp;
   inData.open("yOffset.dat");
   for(int i=0;i<mNTrays;i++) {
     inData >> phi0[i] >> tmp;
   }
   inData.close();
/*   inData.open("/star/institutions/rice/sbutter/run9alignmentsetup/GeomAlign/fitsource/yOffset1.dat");
   for(int i=0;i<mNTrays;i++) {
     inData >> phi1[i] >> tmp;
     phi0[i] += phi1[i];
   }
   inData.close();
  */
   inData.open("xOffset.dat");
   for(int i=0;i<mNTrays;i++) {
     inData >> z0[i] >> tmp >> x0[i] >> tmp;
   }
   inData.close();
/*
   inData.open("/star/institutions/rice/sbutter/run9alignmentsetup/GeomAlign/fitsource/xOffset1.dat");
   for(int i=0;i<mNTrays;i++) {
